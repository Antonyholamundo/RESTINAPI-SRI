...
export class InvoiceService {
  ...
  static async buscarTipoIdentificacion(codigo: string): Promise<IIdentificationType | null> {
    let tipoIdent = await IdentificationType.findOne({ codigo });
    if (!tipoIdent) {
      ...
      const desc = descriptions[codigo] || 'OTRA IDENTIFICACION';
      tipoIdent = new IdentificationType({ codigo, descripcion: desc });
      await tipoIdent.save();
    }
    return tipoIdent;
  }
  ...
  static async buscarProducts(detalles: ProductDetail[]): Promise<IProduct[]> {
    ...
    for (const det of detalles) {
      ...
      let producto = await this.buscarProduct(codigo);
      if (!producto) {
        producto = new Product({
          codigo,
          descripcion: det.detalle.descripcion || 'Producto Autogenerado',
          precio_unitario: parseFloat(det.detalle.precioUnitario) || 0.0,
          tiene_iva: true
        });
        await producto.save();
      }
  ...
  static async procesarFacturaCompleta(datosFactura: InvoiceRequest) {
    ...
    let cliente = await this.buscarClient(datosFactura.infoFactura.identificacionComprador);
    if (!cliente) {
      cliente = new Client({
        tipo_identificacion_id: tipoIdent._id,
        identificacion: datosFactura.infoFactura.identificacionComprador,
        razon_social: datosFactura.infoFactura.razonSocialComprador,
