...
export function generarXMLFactura(
  ...
  const firstDetailImpuesto = factura.detalles[0]?.detalle.impuestos[0]?.impuesto;
  const codigoPorcentaje = firstDetailImpuesto?.codigoPorcentaje || '4';
  const totalImpuestoValor = factura.detalles
    .reduce((acc, item) => acc + parseFloat(item.detalle.impuestos[0].impuesto.valor || '0'), 0)
    .toFixed(2);
  ...
  const doc = create({ version: '1.0', encoding: 'UTF-8' })
    ...
    .ele('totalImpuesto')
    .ele('codigo')
    .txt(firstDetailImpuesto?.codigo || '2')
    .up()
    .ele('codigoPorcentaje')
    .txt(codigoPorcentaje)
    .up()
    .ele('baseImponible')
    .txt(factura.infoFactura.totalSinImpuestos)
    .up()
    .ele('valor')
    .txt(totalImpuestoValor)
    .up()
    .up()
    .up()
    .ele('propina')
  ...
  for (const item of factura.detalles) {
    ...
    doc
      ...
      .ele('impuesto')
      .ele('codigo')
      .txt(item.detalle.impuestos[0].impuesto.codigo || '2')
      .up()
      .ele('codigoPorcentaje')
      .txt(item.detalle.impuestos[0].impuesto.codigoPorcentaje || '4')
      .up()
      .ele('tarifa')
      .txt(item.detalle.impuestos[0].impuesto.tarifa || '15.00')
      .up()
      .ele('baseImponible')
      .txt(d.precioTotalSinImpuesto)
      .up()
